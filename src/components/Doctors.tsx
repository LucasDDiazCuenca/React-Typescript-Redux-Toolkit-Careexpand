...
const Doctors = () => {
	...
	return (
		<>
			<div
				className={`${status.isDeleteConfirmationModalOpen || status.isDoctorDetailModalOpen ? "overlay" : ""}`}
			></div>
			<div className={`w-full`}>
				...
				{doctors.length === 0 && (
					<p className="text-[#818181] font-light text-center">
						There are no doctors at the moment, please create some by clicking in "+ add doctor" button...{" "}
					</p>
				)}
				<div>
					{doctors.map((doctor) => (
